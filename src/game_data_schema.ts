...
function documentation(str:string):string {
	return `<xs:annotation><xs:documentation>${fxml.cdata(str.trim())}</xs:documentation></xs:annotation>`;
}

(async () =>{
	await editorStrings.promise();
	...
	${Object.entries(unparsedEnums).map(([enumName, enumContents]) => {
		...
		tmp += Object.entries(enumContents).map(([k,v]) => {
			return `<xs:enumeration value="${v.name}">${documentation(editorStrings.getKeyValueOrEmpty("EDSTR_ENUMVAL_" + k) + "\n" + editorStrings.getKeyValueOrEmpty("EDSTR_ENUMHINT_" + k))}</xs:enumeration>`;
		}).join('\n');
		...
		return `
			...
			<xs:choice minOccurs="0" maxOccurs="unbounded">
				${Object.entries(fields).map(([fieldName, fieldDesc]) => {
					...
					let doc = documentation(editorStrings.getKeyValueOrEmpty("EDSTR_FIELDNAME_" + structName + "_" + fieldName) + "\n" + editorStrings.getKeyValueOrEmpty("EDSTR_FIELDHINT_" + structName + "_" + fieldName));
					
					if(!isArray){
						return `<xs:element minOccurs="0" maxOccurs="1" name="${fieldName}" type="${fixName(typeName)}">${doc}</xs:element>`;
					}
			...
			${Object.entries(fields).map(([fieldName, fieldDesc]) => {
				...
				if(typeName in accumulatorMapping){
					return `<xs:attribute name="${fieldName}" type="simple_${accumulatorMapping[typeName]}">${doc}</xs:attribute>`;
				}
				
				let isSimpleType = (typeName in unparsedEnums) || (typeName in DataFieldDefaults);
				if(!isSimpleType) return "";
				
				return `<xs:attribute name="${fieldName}" type="simple_${fixName(typeName)}">${doc}</xs:attribute>`;
			}).join("\n")}
